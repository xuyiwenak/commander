...
import { begreatMiniappConfigApi } from '@/api/adminApi';
...
export default function MiniappConfigPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await begreatMiniappConfigApi.get();
  ...
  const handleSave = async (values: FormValues) => {
    ...
    setError(null);
    try {
      await begreatMiniappConfigApi.save(values.baseUrl);
      void message.success('已保存并热加载，下次小程序启动时生效');
    } catch (e) {
      setError(String(e));
    } finally {
      setSaving(false);
    ...
    <div style={{ maxWidth: 560 }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <Text type="secondary" style={{ fontSize: 12 }}>
          修改后立即热加载，无需重启服务。小程序下次拉取配置时生效（最长 24 小时缓存）。
        </Text>
      </div>
        ...
        <Form form={form} layout="vertical" onFinish={(v) => { void handleSave(v); }}>
          <Form.Item
            ...
            rules={[
              ...
              { type: 'url', message: '请输入合法的 URL，如 https://begreat.example.com' },
            ]}
          >
            <Input placeholder="https://begreat.starryspark.com.cn" size="large" />
          </Form.Item>
